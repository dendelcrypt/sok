...
detect_src_ip() {
  # IP used for outgoing connections (usually public IP on VPS)
...
port_free() {
  local p="$1"
  # check TCP listeners; match ":PORT" at end
...
rand_user() {
  # lower+digits, start with u
  echo "u$(tr -dc 'a-z0-9' </dev/urandom | head -c 10)"
}

rand_pass() {
  # avoid characters that often break URL parsers: @ : / % space
  tr -dc 'A-Za-z0-9_.-' </dev/urandom | head -c 18
}

install_pkgs() {
  info "Updating apt & installing packages..."
  apt-get update -y
  apt-get install -y 3proxy curl ca-certificates
}
...
write_cfg() {
...

  {
    ...
    # per-proxy rule: allow user -> socks on port -> flush
    for i in "${!PORTS[@]}"; do
      echo "allow ${USERS[$i]}"
...
test_proxy_local() {
  ...
  # test by reaching external service through local proxy
  # socks5-hostname => DNS through proxy (good for anti-detect behavior)
  local out
...
info "Generating ${COUNT} proxies (random ports, random creds)..."
...
for i in "${!PORTS[@]}"; do
  ...
  if [[ "$res" == OK:* ]]; then
    ...
    ALL_OK=0
    err="${res#FAIL:}"
    echo "  [FAIL] port=${PORTS[$i]} user=${USERS[$i]} (check outbound access / firewall)."
  fi
done
